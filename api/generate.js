...
// Phase AC-V1 FIX: always returns REAL Canva-style poster compositions (elements[] + canvas)
...
/* ===========================
   AC-V1 Poster Composition Engine (server-side materializer)
   Produces real element layouts compatible with the current preview renderer in index.html
=========================== */
...
function paletteForStyle(style, seed) {
  ...
  if (s.includes("light")) {
    pal.bg = "#f8fafc"; pal.bg2 = "#e8eef8";
    pal.ink = "#0b1220"; pal.muted = "#334155";
    pal.accent = base.accent2 || "#2563eb"; pal.accent2 = base.accent || "#0ea5e9";
  } else if (s.includes("corporate")) {
    pal.bg = "#071423"; pal.bg2 = "#0b2a4a";
    pal.ink = "#f3f7ff"; pal.muted = "#b8c7dd";
    pal.accent = "#38bdf8"; pal.accent2 = "#a78bfa";
  } else if (s.includes("neon")) {
    pal.bg = "#05040a"; pal.bg2 = "#130a2a";
    pal.ink = "#ffffff"; pal.muted = "#c7c3ff";
    pal.accent = "#22d3ee"; pal.accent2 = "#fb7185";
...
function buildElements(layout, spec) {
  ...
  if (layout === "splitHero") {
    ...
    add({ type:"pill", x: pad, y: h - pad - Math.round(H2*2.0), w: Math.round(w * 0.30), h: Math.round(H2*2.0), r: 999, fill: pal.accent, text: cta, tcolor: "#0b1020", tsize: clamp(Math.round(H2*0.95), 14, 36), tweight: 800 });

    add({ type:"photo", src: smartPhotoSrc(seed + 11, pal, brand), x: Math.round(w * 0.585), y: Math.round(h * 0.16), w: Math.round(w * 0.32), h: Math.round(h * 0.40), r: 36, stroke: "rgba(255,255,255,0.18)" });
  }

  if (layout === "badgePromo") {
    add({ type:"shape", x: pad, y: pad, w: w - pad*2, h: Math.round(h * 0.64), r: 52, fill: "rgba(255,255,255,0.06)", stroke: "rgba(255,255,255,0.14)" });
    add({ type:"badge", x: w - pad - Math.round(w*0.18), y: pad - Math.round(h*0.01), w: Math.round(w*0.18), h: Math.round(w*0.18), r: 999, fill: pal.accent2, text: "SALE", tcolor:"#0b1020", tsize: clamp(Math.round(H2*0.95), 14, 40), tweight: 900 });
    ...
    // product/photo strip
    add({ type:"photo", src: smartPhotoSrc(seed + 33, pal, "Premium"), x: pad + 10, y: Math.round(h*0.46), w: Math.round(w*0.52), h: Math.round(h*0.22), r: 26, stroke:"rgba(255,255,255,0.14)" });

    add({ type:"pill", x: pad, y: h - pad - Math.round(H2*2.1), w: Math.round(w * 0.38), h: Math.round(H2*2.1), r: 999, fill: pal.accent, text: cta, tcolor:"#0b1020", tsize: clamp(Math.round(H2*0.95), 14, 36), tweight: 900 });
  ...
  if (layout === "minimalQuote") {
    ...
    add({ type:"pill", x: pad + 18, y: h - pad - Math.round(H2*2.1), w: Math.round(w*0.34), h: Math.round(H2*2.1), r: 999, fill: pal.accent2, text: cta, tcolor:"#0b1020", tsize: clamp(Math.round(H2*0.95), 14, 36), tweight: 800 });
...
function makeTemplates({ prompt, category, style, count, divergenceIndex }) {
  ...
  // AC-V1: archetype set with layout hints (used for deterministic layout selection)
  const archetypes = [
  ...
  for (let i = 0; i < count; i++) {
    ...
    const subhead =
      a.vibe === "Info"
        ? (words.length ? "Clear details • Simple structure" : "Clear details • Simple structure")
        : a.vibe === "Urgency"
    ...
    // Materialize a REAL poster composition (AC-V1 fix)
    const composed = materializeTemplate({
      prompt, category, style, i,
      vibe: a.vibe,
